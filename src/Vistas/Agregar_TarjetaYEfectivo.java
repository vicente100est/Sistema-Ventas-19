...
public class Agregar_TarjetaYEfectivo extends javax.swing.JFrame {    
    String codigo,referencia,cantidad,marca,valor;
     
    public Agregar_TarjetaYEfectivo() {
        initComponents();
    ...
    public static void cargarCredito(){
        try{
            ...
            ResultSet r= consulta1.executeQuery("select * from venta_mixta where cod_venta='"+txtCodFactura.getText()+"'");    
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        if (txtTotal.getText().equals("")) {
            ...
            try{
                ...
                ResultSet r= consulta.executeQuery("select cod_venta from venta_mixta where cod_venta='"+txtCodFactura.getText()+"'");
                ...
                int x=0;
                if (banderaCredito==0){
                    x=consulta.executeUpdate("insert into venta_mixta(cod_venta,total,monto_efectivo,monto_tarjeta,saldo_restante,vuelto,cod_caja) VALUES('"+txtCodFactura.getText()+"','"+txtTotal.getText().replace(',','.')+"','"+txtEfectivo.getText().replace(',','.')+"','"+txtTarjeta.getText().replace(',','.')+"','"+txtSaldo.getText().replace(',','.')+"','"+txtVuelto.getText().replace(',','.')+"','"+txtCodCaja.getText()+"')");
                }else{
                    x=consulta.executeUpdate("UPDATE venta_mixta SET total='"+txtTotal.getText().replace(',','.')+"',monto_efectivo='"+txtEfectivo.getText().replace(',','.')+"',monto_tarjeta='"+txtTarjeta.getText().replace(',','.')+"' ,saldo_restante='"+txtSaldo.getText().replace(',','.')+"' ,vuelto='"+txtVuelto.getText().replace(',','.')+"', cod_caja='"+txtCodCaja.getText()+"' WHERE cod_venta='"+txtCodFactura.getText()+"'");
                }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Agregar_TarjetaYEfectivo.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Agregar_TarjetaYEfectivo().setVisible(true);
            }
        });
    }
