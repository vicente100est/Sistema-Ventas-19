...
public class Estadistica_Compras extends javax.swing.JDialog {
    int Select,bandera;
    ...
    public Estadistica_Compras(java.awt.Frame parent, boolean modal) {
        //super(parent, modal);
        initComponents();
    ...
    public void cargarTablaFacturas(){
         try{
            ...
            ResultSet r= consulta.executeQuery("SELECT c.cod_compra, c.fecha, c.tipo_pago,c.total_con_iva,c.total_sin_iva,c.tipo_factura FROM compra c WHERE c.fecha >= '"+fecha1+"' AND c.fecha <= '"+fecha2+"' group by c.cod_compra order by c.fecha ASC");
            total=0.0;
            totalBrutoFact=0.0;
            int i,j;
            i=0;
            ...
            Statement consulta1=conn.createStatement();
            ResultSet r5= consulta1.executeQuery("select SUM(total_con_iva) from compra where fecha BETWEEN '"+fecha1+"' AND '"+fecha2+"'");
            while(r5.next()){
               total=Double.parseDouble(r5.getString(1));
            }
    ...
    public void cargarTablaFacturas22(){
         try{
            ...
            ResultSet r= consulta.executeQuery("SELECT c.fecha, sum(c.total_con_iva) FROM compra c WHERE c.fecha >= '"+fecha1+"' AND c.fecha <= '"+fecha2+"' group by c.fecha order by c.fecha ASC");
            total=0.0;
            int i,j;
            i=0;
            j=0;
    ...
    public void cargarTablaFacturasMes(){
         try{
            ...
            ResultSet r= consulta.executeQuery("SELECT c.cod_compra, c.fecha, c.tipo_pago,c.total_con_iva,c.total_sin_iva, c.tipo_factura FROM compra c WHERE  MONTH(c.fecha) >= '"+fecha1+"' and YEAR(c.fecha) >= '"+añoDesde+"' AND MONTH(c.fecha) <= '"+fecha2+"' and YEAR(c.fecha) <= '"+añoHasta+"' group by c.cod_compra");
            total=0.0;
            totalBrutoFact=0.0;
            int i,j;
            i=0;
            ...
            Statement consulta1=conn.createStatement();
            ResultSet r5= consulta1.executeQuery("select SUM(total_con_iva) from compra where MONTH(fecha) >= '"+fecha1+"'AND YEAR(fecha) >= '"+añoDesde+"' AND MONTH(fecha) <= '"+fecha2+"' AND YEAR(fecha) <= '"+añoHasta+"'");
            while(r5.next()){
               total=Double.parseDouble(r5.getString(1));
            }
    ...
    public void cargarTablaFacturasMes22(){
         try{
            ...
            ResultSet r= consulta.executeQuery("SELECT MONTH(c.fecha),YEAR(c.fecha), sum(c.total_con_iva) FROM compra c WHERE MONTH(c.fecha) >= '"+fecha1+"' and YEAR(c.fecha) >= '"+añoDesde+"' AND MONTH(c.fecha) <= '"+fecha2+"' and MONTH(c.fecha) <= '"+añoHasta+"' group by MONTH(c.fecha)");
            total=0.0;
            int i,j;
            i=0;
            j=0;
    ...
    public void graficarDias(){
        ...
        JFreeChart ch =ChartFactory.createBarChart3D("GRÁFICA DECOMPRAS POR DÍA", "DÍAS", "MONTO", dtsc, PlotOrientation.VERTICAL, true, true, false);
    ...
    public void graficarArticulos(){
        ...
        JFreeChart ch =ChartFactory.createBarChart3D("GRÁFICA DE COMPRAS POR DÍA", "DÍAS", "MONTO", dtsc, PlotOrientation.VERTICAL, true, true, false);
        ChartPanel cp= new ChartPanel(ch);
        panelEstadistica.add(cp);
        cp.setBounds(0,0,630,420);
    }
    ...
    private void initComponents() {
        ...
        panelTope.add(jButton1, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 20, -1, -1));
        ...
        jLabel10.setText("ESTADÍSTICA DE COMPRAS");
        panelTope.add(jLabel10, new org.netbeans.lib.awtextra.AbsoluteConstraints(90, 30, 380, -1));
        ...
        jLabel12.setText("Compras por Mes");
        panelTope.add(jLabel12, new org.netbeans.lib.awtextra.AbsoluteConstraints(120, 490, 240, -1));

        jLabel7.setFont(new java.awt.Font("Calibri", 1, 30)); // NOI18N
        jLabel7.setText("Compras por Día");
        panelTope.add(jLabel7, new org.netbeans.lib.awtextra.AbsoluteConstraints(128, 280, 230, -1));
        ...
        txtGananciaNeta.setBounds(220, 190, 140, 40);

        labelGananciaNeta.setFont(new java.awt.Font("Calibri", 1, 20)); // NOI18N
        labelGananciaNeta.setText("Total IVA");
        panelEsta.add(labelGananciaNeta);
        labelGananciaNeta.setBounds(130, 190, 90, 40);
        ...
        txtMargenGanancia.setBounds(220, 240, 140, 40);

        jLabel15.setFont(new java.awt.Font("Calibri", 1, 20)); // NOI18N
        jLabel15.setText("Total con IVA");
        panelEsta.add(jLabel15);
        jLabel15.setBounds(370, 240, 120, 40);
        ...
        labelTotalBruto.setText("Total sin IVA");
        panelEsta.add(labelTotalBruto);
        labelTotalBruto.setBounds(370, 190, 120, 40);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Estadistica_Compras.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                Estadistica_Compras dialog = new Estadistica_Compras(new javax.swing.JFrame(), true);
