...
public class Agregar_Credito extends javax.swing.JFrame {    
    ...
    public static void cargarCredito(){
        try{
            ...
            ResultSet r= consulta1.executeQuery("select * from venta_credito where cod_venta='"+txtCodFactura.getText()+"'");    
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        if (txtTotalCredito.getText().equals("")) {
            ...
            try{
                ...
                ResultSet r= consulta.executeQuery("select cod_venta from venta_credito where cod_venta='"+txtCodFactura.getText()+"'");
                ...
                int xx=0;
                if (banderaCredito==0){
                    xx=consulta.executeUpdate("insert into venta_credito(cod_venta,total,acuenta,saldo_restante,cod_caja) VALUES('"+txtCodFactura.getText()+"','"+txtTotalCredito.getText().replace(',','.')+"','"+txtAcuenta.getText().replace(',','.')+"','"+txtSaldo.getText().replace(',','.')+"','"+txtCodCaja.getText()+"')");
                }else{
                    xx=consulta.executeUpdate("UPDATE venta_credito SET total='"+txtTotalCredito.getText().replace(',','.')+"',acuenta='"+txtAcuenta.getText().replace(',','.')+"',saldo_restante='"+txtSaldo.getText().replace(',','.')+"', cod_caja='"+txtCodCaja.getText()+"' WHERE cod_venta='"+txtCodFactura.getText()+"'");
                }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Agregar_Credito.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Agregar_Credito().setVisible(true);
            }
        });
    }
